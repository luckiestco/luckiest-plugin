---
name: luckiest-ai-seo
description: "When the user wants to optimize content for AI search engines, get cited by LLMs, or appear in AI-generated answers. Also use when the user mentions 'AI SEO,' 'AEO,' 'GEO,' 'LLMO,' 'answer engine optimization,' 'generative engine optimization,' 'LLM optimization,' 'AI Overviews,' 'optimize for ChatGPT,' 'optimize for Perplexity,' 'AI citations,' 'AI visibility,' 'zero-click search,' 'how do I show up in AI answers,' 'LLM mentions,' 'optimize for Claude/Gemini,' 'llms.txt,' 'AGENTS.md,' 'OKF,' 'Open Knowledge Format,' 'knowledge bundle,' or 'agent-readable site.' Use this whenever someone wants their content to be cited or surfaced by AI assistants and AI search engines. For traditional technical and on-page SEO audits, see luckiest-seo-audit. For structured data implementation, see luckiest-schema."
license: See ATTRIBUTION.md
metadata:
  version: "1.1.0"
  listing_id: luckiest-ai-seo
  author: luckiest
---

# Luckiest AI SEO

This skill provides expertise in AI search optimization — the practice of making content discoverable, extractable, and citable by AI systems including Google AI Overviews, ChatGPT, Perplexity, Claude, Gemini, and Copilot. The goal is to help users get their content cited as a source in AI-generated answers.

## Staying current

On activation, call the Luckiest MCP check_updates tool with { listingId: "658062d1-7ff2-498f-a355-0ed2e2a516fb", installedSemver: "1.1.0" }. If it returns upToDate: false, surface the notice to the user once, then continue. Do nothing further if upToDate: true. Never block on this check — if the tool is unavailable, proceed.

When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-ai-seo", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
### Audit non-determinism

AI answers are non-deterministic — the same query can cite different sources across sessions, accounts, regions, and days. A single check is a snapshot, not a verdict. Run each priority query 2-3 times (ideally on different days or from a logged-out session) before concluding whether a brand is cited. Record the citation rate ("cited in 2 of 3 runs"), not a single yes/no.

### Share the audit with your tribe (offer, never automatic)

Once the AI Visibility Audit is complete, offer to surface it to the user's Luckiest tribe: "Want me to share these AI-visibility findings — the query set, who's getting cited, and the gaps — with your Luckiest tribe? Anyone auditing the same competitors can pick up where you left off." Only share if the user accepts. Never auto-post, never include credentials or private analytics, and skip the offer for confidential competitive work.
...
**`/AGENTS.md`** — Capability file for autonomous agents

As coding and buying agents increasingly act on a site directly, an `AGENTS.md` at the repo or site root declares what actions an agent can take, where the canonical pricing/spec files live, and any usage constraints. Treat it the same way as `llms.txt`: no confirmed ranking signal, but it is trivially parseable and helps agent-mediated workflows resolve to your product instead of skipping it. Keep it short and link it from the sitemap.
...
Content with proper schema shows 30-40% higher AI visibility on non-Google AI engines. **Google's note**: structured data is "not required for generative AI search" but is recommended for overall SEO strategy. For implementation, use the **luckiest-schema** skill.
...
- **luckiest-seo-audit**: For traditional technical and on-page SEO audits
- **luckiest-schema**: For implementing structured data that helps AI understand your content
- **luckiest-content-strategy**: For planning what content to create
- **luckiest-competitors**: For building comparison pages that get cited
- **luckiest-programmatic-seo**: For building SEO pages at scale
- **luckiest-copywriting**: For writing content that's both human-readable and AI-extractable
