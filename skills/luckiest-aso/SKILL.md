---
name: luckiest-aso
description: "When the user wants to audit or optimize an App Store or Google Play listing. Also use when the user mentions 'ASO audit,' 'app store optimization,' 'app store audit,' 'Play Store listing,' 'grade my app listing,' 'optimize my app listing,' 'improve app visibility,' 'app store ranking,' 'audit my listing,' 'why aren't people downloading my app,' 'improve my app conversion,' 'keyword optimization for app,' or 'compare my app to competitors.' Use when the user shares an App Store or Google Play URL and wants to improve it."
license: See ATTRIBUTION.md
metadata:
  version: "1.1.0"
  listing_id: luckiest-aso
  author: luckiest
---

# ASO Audit (Luckiest edition)
...
When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-aso", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
**Cold-start exception (any tier):** If the app is brand-new — very few ratings (roughly <50) or a bottom download range — do not dock the Ratings & Reviews dimension for low *volume*. A new listing has not had time to accumulate reviews. Score that dimension on rating *quality* and review *recency* relative to the app's age and install range, and note that volume will build. Docking a two-week-old app for having 12 reviews is a false negative.
...
After delivering the audit, offer (never auto-post): "Want me to surface this ASO audit to your Luckiest tribe? Anyone working on an app in the same category can see the wins and keyword gaps this listing showed." Only share if the user accepts. Never post credentials, App Store Connect / Play Console data, or a competitor's private analysis without an explicit ask. If the user declines, stop.

## Related Skills

- **luckiest-cro**: For optimizing the conversion of web-based landing pages that drive app installs
- **luckiest-ad-creative**: For creating App Store and Google Play ad creatives
- **luckiest-analytics**: For setting up install attribution and in-app event tracking
- **luckiest-customer-research**: For understanding user needs and language to inform listing copy
