...
Full template library for Luckiest Prompt Rewrite. Read the relevant template when the user's task type matches. Do not load all templates at once — only the one you need.
...
| [M — Opus Task Brief](#template-m--opus-task-brief) | Complex, multi-step, or agentic task on Claude Opus 4.8 / Claude 5 |
...
**Variant note:** whenever the `Experiment` line asks for multiple variants, invoke the `luckiest-thinker` skill before writing them so the options span real diversity across independent axes, not three phrasings of the same idea.
...
*Use for any complex, multi-step, or agentic task on Claude Opus 4.8 / Claude 5 — claude.ai, API, or Claude Code. Modern Opus reads prompts literally. Missing context produces narrow output. This template front-loads everything so the first turn is the only turn.*
...
**When to use:** Opus 4.8 / Claude 5 on any surface — claude.ai, API, Claude Code — when the task is complex, multi-file, ambiguous, or agentic. Not needed for simple one-shot tasks.
