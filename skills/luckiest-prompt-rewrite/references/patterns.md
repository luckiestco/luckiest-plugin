...
| 36 | **Vague first turn on modern Opus** | "fix the auth bug" with no scope, no files, no criteria | Modern Opus reads prompts literally — it does not fill implicit context. Use Template M. Front-load intent, file scope, constraints, and acceptance criteria. |
