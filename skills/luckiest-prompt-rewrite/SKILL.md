---
name: luckiest-prompt-rewrite
description: Rewrites a rough idea or an existing prompt into a single production-ready prompt optimized for a specific AI tool. Activates only when the user explicitly asks to write, fix, improve, rewrite, adapt, split, or migrate a prompt for a named AI tool (LLM, Cursor, Claude Code, Midjourney, image/video AI, coding agents, etc.). Trigger phrases include "write me a prompt", "fix this prompt", "improve my prompt", "rewrite this for [tool]", "adapt this prompt for [tool]", "/luckiest-prompt-rewrite". Does not activate for general conversation, coding tasks, document writing, or other non-prompt work.
...
**Claude (claude.ai, Claude API, Claude Opus 4.8 / Claude 5 family / Claude 4.x)**
- Be explicit and specific — modern Claude models follow instructions literally. Opus 4.8 especially: it does exactly what you say, nothing more. Missing context = narrow literal output, not a smart guess.
...
- Do NOT add "think step by step" or fixed thinking budget instructions — current Claude models use adaptive thinking and calibrate depth automatically. To influence depth: "Think carefully before responding" (more) or "Prioritize responding quickly" (less).
- Use Template M for agentic or multi-step tasks.
- **Fable 5 gate:** if the target is Claude/Claude Code and the model is unspecified, ask once — "Are you planning to run this on Claude Fable 5 (or Mythos 5)?" (counts toward the 3-question limit). If yes, use the Fable 5 block below. If they want help choosing a model, point them to the `model-router` skill. If no or unknown, use the standard Claude guidance above.

---

**Claude Fable 5 / Mythos 5** (only when the user confirms this is the target model)
Fable 5 follows brief, intent-level instructions better than Opus 4.8 — steer with one short instruction instead of enumerating every behavior. Apply only what the prompt needs:
- **Longer turns by default.** Hard tasks run for minutes; autonomous runs for hours. For ambiguous tasks, add: "When you have enough information to act, act. Give a recommendation, not an exhaustive survey. (Does not apply to thinking blocks.)"
- **Effort is the main control**, not prompt scaffolding — `high` default, `xhigh` for the hardest work, `medium`/`low` for routine. Do not hand-tune thinking depth.
- **Over-engineering guard (higher effort):** "Don't add features, refactor, abstractions, error handling, or backwards-compat shims beyond what the task requires. Do the simplest thing that works. Only validate at system boundaries."
- **Brevity:** "Lead with the outcome — your first sentence answers 'what happened'. Be selective about what you include, don't compress into fragments or arrow chains."
- **Ground progress claims (long runs):** "Before reporting progress, audit each claim against a tool result from this session. Report only what you can point to evidence for."
- **State boundaries:** "When the user is describing a problem or thinking out loud rather than requesting a change, report findings and stop. Don't apply a fix until asked."
- **Subagents:** delegate readily and asynchronously — "Delegate independent subtasks to subagents and keep working while they run."
- **Do NOT tell it to echo, transcribe, or explain its internal reasoning as response text** — this triggers a refusal and falls back to Opus 4.8. Remove any "show your thinking" instruction when migrating a prompt to Fable 5.
- **Don't over-prescribe.** Skills and prompts written for older models are often too prescriptive for Fable 5 and degrade output — strip enumerated behavior lists down to intent.
...
- Opus 4.8 default in Claude Code runs high effort — do NOT specify effort level in prompts, it's already set
- Opus 4.8 is literal — vague first turns produce narrower results. Front-load everything: intent, file scope, constraints, acceptance criteria, session strategy.
- Opus 4.8 uses fewer tool calls by default and reasons more between calls — explicitly instruct tool use when needed: "Read all files in /src/auth/ before starting"
- Opus 4.8 spawns fewer subagents by default — explicitly request when needed: "Use a subagent to investigate X so it stays out of main context"
...
**Model Migration Mode**
Detect when: user has a prompt that worked on one model and wants it to work on a different one (e.g. "this worked on GPT-4, make it work on Opus 4.8", "port my o3 prompt to Claude").
This is an Adapt task — preserve intent, re-fit syntax and technique to the target's routing rules above.
Critical re-fits when migrating: strip CoT scaffolding when moving TO a reasoning-native model (o3, R1, Qwen3-thinking); add explicit structure when moving to weaker instruction-followers (Llama, Mistral); add literal-scope locks and "only what was asked" when moving to Claude Opus; when moving TO Claude Fable 5 / Mythos 5, strip enumerated behavior lists down to intent-level instructions and remove any "show/explain your reasoning" instruction (it triggers a refusal). See the Claude Fable 5 / Mythos 5 routing block.
Use Template L (Adapt output format).
...
**Variant diversity** — when the request asks for multiple variants (CRISPE "give 3 variants", A/B headline sets, name/persona lists, or creative-domain prompts for copywriting, image, or video), invoke the `luckiest-thinker` skill before writing them. It counteracts mode collapse so the variants span real diversity across independent axes, instead of three near-identical takes on the same obvious answer. Skip it for single-prompt requests — one production-ready prompt does not need a distribution.
...
After delivering a prompt the user is happy with, you may offer once: "Want me to share this prompt with your Luckiest tribe so members working on the same tool can reuse it?" This is an offer the user can decline — never auto-post. Do not share prompts that contain private context, secrets, or personal data. Reference env var names only, never values.
