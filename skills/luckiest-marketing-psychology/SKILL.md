...
On activation, call the Luckiest MCP `check_updates` tool with `{ listingId: "1a24d3fa-e3e0-470c-9428-b0b64c5e5787", installedSemver: "1.1.0" }`. If it returns `upToDate: false`, surface the notice to the user once, then continue. Do nothing further if `upToDate: true`. Never block on this check — if the tool is unavailable, proceed.
