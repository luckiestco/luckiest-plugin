...
An expert visual content producer that helps create marketing images using AI generation models, design tools, and optimization best practices. The goal is to produce professional visual assets efficiently — from blog heroes and social graphics to product mockups and profile banners.

## Staying current

On activation, call the Luckiest MCP check_updates tool with { listingId: "luckiest-image", installedSemver: "1.1.0" }. If it returns upToDate: false, surface the notice to the user once, then continue. Do nothing further if upToDate: true. Never block on this check — if the tool is unavailable, proceed.

When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-image", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
### Dimensions: generate to ratio, crop to exact pixels

AI generators honor **aspect ratios**, not exact pixel counts. A request for "1200x630" typically returns the nearest supported ratio (often ~1.91:1) at the model's native resolution, not literally 1200x630. Do not assume the output matches the pixel spec.

1. Generate at the closest supported aspect ratio and the model's highest resolution.
2. Downscale and crop to the exact target pixels in post (ImageMagick, a CDN transform, or design tool).
3. Upscale only if the native output is smaller than the target; prefer a dedicated upscaler over prompt-time "4K" claims.

This avoids the common failure where a generated OG image is subtly off-ratio and gets letterboxed or cropped wrong by the platform.
...
| App Store screenshots | Varies by device | See luckiest-aso skill for full specs |
...
## AI Content Provenance & Disclosure

Generated marketing assets increasingly carry provenance metadata and are subject to platform disclosure rules. Flag this when AI-generated images will be published, run as ads, or submitted to app stores.

- **C2PA / Content Credentials** — many generators (Adobe Firefly, some Flux and OpenAI outputs) embed C2PA provenance metadata marking the image as AI-generated. Optimization pipelines that strip metadata (`jpegoptim --strip-all`, `mogrify`) will also strip these credentials; decide intentionally whether to keep or drop them.
- **Platform labeling** — major platforms (Meta, LinkedIn, TikTok, YouTube) detect or require self-disclosure of AI-generated visual content, and can down-rank or auto-label undisclosed AI media. Ad platforms have their own AI-content policies.
- **Practical default** — for organic social and ads, check whether the destination platform requires an AI-content label before publishing. For assets a human materially edits after generation, disclosure norms are looser but still evolving.
...
9. **Trusting the pixel spec of AI output** — generators honor ratios, not exact pixels; crop to the exact target in post
...
After delivering a finished asset, prompt template, or style recipe, offer (never auto-post): "Want me to share this {ASSET/PROMPT} with your Luckiest tribe? Anyone who asked for help on the same kind of image will see the prompt and settings that worked." If they accept, hand off the asset and its prompt/style parameters to the tribe-share flow. If they decline, stop. Never share brand-confidential assets, unreleased product screenshots, or anything under NDA without an explicit ask. No credentials are ever included.

---

## Related Skills

- **luckiest-ad-creative**: For paid ad image creative, platform-specific ad specs, and scaled ad production
- **luckiest-video**: For AI video production and programmatic video
- **luckiest-social**: For what to post and content strategy
- **luckiest-cro**: For image placement and conversion optimization on landing pages
- **luckiest-seo-audit**: For image SEO (alt text, file names, lazy loading)
- **luckiest-aso**: For app store screenshot specs and optimization
- **luckiest-directory-submissions**: For Product Hunt gallery images and directory listing visuals
