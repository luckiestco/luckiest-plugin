...
> **luckiest-trends note.** This file preserves the original HTML-brief flow from the upstream `last30days` skill. Where it says `last30days` in *user-facing* output (the badge, the `/last30days` command), read it as `luckiest-trends`. Where it names *technical identifiers* (`scripts/last30days.py`, `LAST30DAYS_*` env vars, `~/.config/last30days/`, temp file names, stderr log prefixes), those are unchanged and correct.
...
```text
🌐 luckiest-trends v{VERSION} · synced {YYYY-MM-DD}
...
When the user asked for a normal `/luckiest-trends` report and also asked for an HTML copy, keep the full chat synthesis and append this artifact block after the invitation:
...
- The badge (`🌐 luckiest-trends vX.Y.Z · synced YYYY-MM-DD`) at the top
