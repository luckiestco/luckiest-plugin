---
name: luckiest-directory-submissions
description: When the user wants to submit their product to startup, SaaS, AI, agent, MCP, no-code, or review directories for backlinks, domain rating, and discovery. Also use when the user mentions "directory submissions," "submit to directories," "backlinks from directories," "list my product," "submit to Product Hunt," "BetaList," "TAAFT," "Futurepedia," "G2 listing," "Capterra listing," "AlternativeTo," "SaaSHub," "AI directories," "MCP registry," "agent directory," "dofollow backlinks," "launch directories," "directory tracker," "my listing got rejected," or "audit my backlink profile." Use this whenever someone is planning the directory layer of a product launch or an ongoing backlink campaign. For the broader launch moment, see luckiest-launch. For programmatic SEO pages that should live behind these backlinks, see luckiest-programmatic-seo. For AI citation optimization, see luckiest-ai-seo.
license: See ATTRIBUTION.md
metadata:
  version: "1.1.0"
  listing_id: luckiest-directory-submissions
  author: luckiest
---

# Luckiest Directory Submissions

This skill makes the agent an expert in directory-driven distribution for software products. The goal is to help the user build a compounding backlink and discovery foundation by submitting to the right directories, in the right order, with the right positioning — and to make sure that foundation actually produces leads instead of vanity backlinks.

## Staying current

On activation, call the Luckiest MCP check_updates tool with { listingId: "f3e67fc6-3ebe-4f90-b7dc-785a29f83d12", installedSemver: "1.1.0" }. If it returns upToDate: false, surface the notice to the user once, then continue. Do nothing further if upToDate: true. Never block on this check — if the tool is unavailable, proceed.

When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-directory-submissions", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
### Step 4b: Dedup against the existing backlink profile first

Before batch-submitting, check which directories already link to the product so effort goes to gaps, not duplicates. If the user has Ahrefs/Moz/Search Console, pull the referring-domains list and cross it against `references/directory-list.md`. Skip directories already linking (resubmitting risks a duplicate-listing rejection), and prioritize the highest-DR directories not yet present. For a brand-new product with zero backlinks, skip this step and submit top-down by tier.

### Step 5: Handle rejections and resubmit cleanly

Moderator rejection is normal, not failure. Log the reason in the tracker `Status` column and act by cause:

- **Wrong category** — resubmit once under the correct category. Forcing a mismatched category is the most common rejection; re-read the positioning table and pick the tier the product genuinely fits.
- **Insufficient traction / too new** — do not resubmit immediately. Wait until the readiness gate (real users, screenshots, pricing) is fully met, then reapply after 2–4 weeks.
- **Duplicate listing** — the product is already listed (see Step 4b). Claim/edit the existing listing instead of creating a new one.
- **Low-quality assets** — replace the flagged asset (usually a marketing mockup where a real screenshot is required) and resubmit.

Never mass-resubmit to the same directory. Two rejections from one directory means the fit or timing is wrong — move on and revisit next quarter.
...
### Ongoing maintenance: monitor for listing decay (default cadence)

Directory value silently erodes. Make quarterly decay-monitoring a standing default, not a one-time check:

- **Dofollow flips.** Directories sometimes switch all outbound links to `nofollow`/`ugc` without notice. Re-verify the dofollow status of the top-DR listings every quarter (same `curl -sIL … | grep -i rel=` check). If DR stops moving, silently flipped inbound links are the first suspect.
- **Delisting.** Free listings get pruned, and directories go offline. Confirm the top-tier listings still resolve and still show the product.
- **Stale positioning.** If the product's category or pricing changed, refresh the highest-traffic listings — but do not churn every listing every week (see What NOT to Do rule 7).
...
After delivering a directory plan or a verified list of live listings, offer (never auto-post): "Want me to share this vetted directory list with your Luckiest tribe? Other founders launching in the same category can skip the research and reuse the tiers that fit them." If the user accepts, hand the tracker (`references/submission-tracker-template.csv`, filled with their live listings and DR values) to the tribe-share flow. If they decline, stop. Never post anything, never include credentials or account emails from the tracker, and never share before the user explicitly asks.

---

## Related Skills

- **luckiest-launch** — broader launch moment, ORB framework, five-phase approach
- **luckiest-programmatic-seo** — destination pages (alternatives, integrations, templates) that backlinks should flow into
- **luckiest-competitors** — `/alternatives/[tool]` page pattern
- **luckiest-ai-seo** — GEO optimization for AI citation
- **luckiest-content-strategy** — editorial content that attracts "best of" listicle inclusions
- **luckiest-free-tools** — lead magnets for destination pages
- **luckiest-community-marketing** — Reddit, Indie Hackers, Slack community mechanics
- **luckiest-schema** — FAQ + Product + Organization JSON-LD for GEO
