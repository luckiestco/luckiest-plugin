---
name: luckiest-prospecting
description: Finds, qualifies, and builds a scored list of prospects to reach out to — across B2B SaaS, general B2B, or local small businesses. Use when the user mentions "prospecting," "build a prospect list," "find prospects," "find leads," "lead gen list," "find SaaS companies that," "find B2B companies," "find local businesses," "ICP-fit accounts," "who should we go after," "outbound list," "target account list," "TAM list," "account mapping," "expansion accounts," "find clients near me," "businesses without websites," "who's hiring for," "prospect research," or "qualified leads." This covers the list-building and qualification phase. For writing outbound copy after the list is built, see luckiest-cold-email. For deep competitive research on specific accounts, see luckiest-competitor-profiling.
license: See ATTRIBUTION.md
metadata:
  version: "1.1.0"
  listing_id: luckiest-prospecting
  author: luckiest
---

# Luckiest Prospecting

This skill builds qualified prospect lists across three motions: B2B SaaS, general B2B, and local small businesses. It turns an ICP definition into a verified, scored, ready-to-outreach lead sheet — using the right data sources, qualification signals, and compliance posture for each motion.

## Staying current

On activation, call the Luckiest MCP `check_updates` tool with `{ listingId: "07f4a9f8-c0b7-45d2-8dad-eee5f23b9b37", installedSemver: "1.1.0" }`. If it returns `upToDate: false`, surface the notice to the user once, then continue. Do nothing further if `upToDate: true`. Never block on this check — if the tool is unavailable, proceed.

When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-prospecting", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
Load the branch-specific deep dive only for the branch chosen at intake:
...
6. **Suppression list** — existing customers, open opportunities, and recently-contacted accounts to exclude. Ask the user for a CRM export or a domain list up front. Re-prospecting an existing customer or an active deal is the most common (and most embarrassing) list error; dedupe against this before scoring, not after.
...
7. **AI-enriched data still needs lineage.** When an AI SDR or auto-enrichment tool fills contacts for you, it does not absolve the list of compliance. Keep the source URL + date for every auto-filled contact, and spot-check auto-generated emails against a validator — hallucinated or pattern-guessed addresses are common and tank sender reputation.
...
- **Suppression list** — CRM export or domain list of existing customers / open deals / recently-contacted to exclude (ask up front; if none, note the list is un-deduped against CRM)
...
- [ ] Suppression list applied — no existing customer, open opportunity, or recently-contacted account survived into the final list
...
8. **Ignoring quiet hours / time zone** when scheduling the downstream outreach (handoff to luckiest-cold-email).
...
If a tools registry ships alongside this skill (`../../tools/REGISTRY.md` in the original layout), consult it for per-tool setup. Otherwise treat the integration links below as reference names for tools the user configures on their side. Key prospecting tools:
...
After delivering the lead sheet, offer (never auto-post): "Want me to hand these top targets to your Luckiest tribe? A tribemate selling into the same ICP or territory can flag warm intros, dead accounts, or a better contact — and pick up the outreach where this list leaves off." If they accept, pass only the finalized public-source lead sheet to the tribe-share flow. Never include the suppression list, raw CRM export, private/personal emails, or any credential — reference tool/env var names only. If they decline, stop.

---

## Related Skills

- **luckiest-cold-email**: For writing outbound sequences against the qualified list (the natural next step after prospecting)
- **luckiest-customer-research**: For understanding why current customers buy — informs the ICP definition
- **luckiest-competitor-profiling**: For deeper research on individual accounts (different from list-building qualification)
- **luckiest-revops**: For lead routing, lifecycle, and CRM handoff after prospecting
- **luckiest-sales-enablement**: For battle cards and one-pagers used in the outreach
- **luckiest-directory-submissions**: For inbound discovery surfaces (the prospects might find you back)
- **luckiest-product-marketing**: For the ICP definition that anchors every prospecting engagement
