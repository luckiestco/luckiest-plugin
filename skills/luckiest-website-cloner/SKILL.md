...
description: >-
  ...
  DOM vs GPU-rendered, routes plain layout to the luckiest-website-cloner-dom sub-skill and shader
  surfaces to the luckiest-website-cloner-shaders sub-skill, scaffolds a build target on demand,
...
metadata:
  version: "1.0.0"
  listing_id: luckiest-website-cloner
  author: luckiest
---
...
- **`../luckiest-website-cloner-dom/SKILL.md`** — extracts DOM/CSS/assets/content section by
  section, writes a spec file per component, dispatches parallel builders.
- **`../luckiest-website-cloner-shaders/SKILL.md`** — evidence-gated capture-and-replay of a
...
1. **Browser automation is required.** You need a browser you can drive from
   ...
   preload; see `references/builtin-browser-recipes.md`). If none can load the target (bot wall, auth), stop and say so —
   this skill cannot clone from a fetch or a screenshot.
   Build the injectable probe bundle with `node scripts/build-bundle.cjs`, inject
...
3. **Read the routing summary.** For each surface:
   - `WEBGL1 / WEBGL2 / WEBGPU` → **luckiest-website-cloner-shaders track**. This is a GPU effect
     the DOM track literally cannot capture.
   - `CANVAS2D` / `CANVAS_UNKNOWN` → inspect. Many 2D canvases are decorative
     particle fields (luckiest-website-cloner-shaders) but some are charting libs better rebuilt
     from data. Use judgment; default animated fullscreen 2D → luckiest-website-cloner-shaders.
   - `VIDEO` → **luckiest-website-cloner-dom track**, re-embed the source `<video>`. Do not rebuild
     a looping video as a canvas — that's wasted effort and looks worse.
   - `SVG_ANIMATED` → luckiest-website-cloner-dom, but preserve the SMIL/CSS animation.
   - everything else (all normal DOM) → **luckiest-website-cloner-dom track**.
4. **Record the interaction model.** The surface map also fingerprints smooth
   ...
   how luckiest-website-cloner-dom must rebuild scroll-driven sections. Confirm by scrolling: does
...
6. **Probe the design system** — run `scripts/tokens-probe.js` (`tokensProbe()`)
   ...
   These values ARE the design tokens luckiest-website-cloner-dom writes in its foundation step.
...
proceed with just the luckiest-website-cloner-dom track and skip the luckiest-website-cloner-shaders phase. Say so.
...
- Static/marketing page, GPU effects present → **Vite + vanilla/TS** (the
  luckiest-website-cloner-shaders baselines are framework-free; Vite composites them cleanly).
- App-like, component-heavy, needs routing/SSR → **Next.js + Tailwind** (matches
  the luckiest-website-cloner-dom builder conventions).
...
0.02–0.06% pixel diff in one pass. Otherwise run luckiest-website-cloner-dom's rebuild. State the
...
**DOM track:** Follow `../luckiest-website-cloner-dom/SKILL.md`. Feed it the section topology from
...
through the ladder in `../luckiest-website-cloner-dom/references/asset-resolution.md` — REAL first,
...
**GPU track:** For each GPU surface, follow `../luckiest-website-cloner-shaders/SKILL.md` once,
...
- GPU surfaces found, their drivers, and replay fidelity (SOURCE / PARTIAL /
  approximate — carry luckiest-website-cloner-shaders's honesty labels through; don't upgrade them)
...
- **Yes** → invoke the `luckiest-website-cloner-remix` skill with this run's output directory
  (`Skill: luckiest-website-cloner-remix`, args `<output-dir>`). It consumes TEARDOWN.md,
...
- **No / later** → end with: `/luckiest-website-cloner-remix <output-dir>` re-enters at any time.

Do not start re-skinning inside luckiest-website-cloner; luckiest-website-cloner ends at a verified copy
and luckiest-website-cloner-remix owns everything after.
## Guardrails
