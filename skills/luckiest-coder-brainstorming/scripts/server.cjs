...
function brandMarkup() {
  ...
  const text = SUPERPOWERS_TELEMETRY_DISABLED
    ? 'Luckiest Coder v' + version
    : 'Luckiest Coder v' + version;
  // Luckiest Coder edition: do not render the upstream third-party brand logo.
  const logo = '';

  return '<div class="brand"><span class="brand-copy">' + logo + text + '</span></div>';
}
