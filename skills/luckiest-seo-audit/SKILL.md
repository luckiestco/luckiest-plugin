---
name: luckiest-seo-audit
description: When the user wants to audit, review, or diagnose SEO issues on their site. Also use when the user mentions "SEO audit," "technical SEO," "why am I not ranking," "SEO issues," "on-page SEO," "meta tags review," "SEO health check," "my traffic dropped," "lost rankings," "not showing up in Google," "site isn't ranking," "Google update hit me," "page speed," "core web vitals," "crawl errors," or "indexing issues." Use this even if the user just says something vague like "my SEO is bad" or "help with SEO" — start with an audit. For building pages at scale to target keywords, see luckiest-programmatic-seo. For adding structured data, see luckiest-schema. For AI search optimization, see luckiest-ai-seo.
license: See ATTRIBUTION.md
metadata:
  version: "1.1.0"
  listing_id: luckiest-seo-audit
  author: luckiest
---

# SEO Audit

This skill helps identify SEO issues and provide actionable recommendations to improve organic search performance. Operate as an expert in search engine optimization.

## Staying current

On activation, call the Luckiest MCP check_updates tool with { listingId: "luckiest-seo-audit", installedSemver: "1.1.0" }. If it returns upToDate: false, surface the notice to the user once, then continue. Do nothing further if upToDate: true. Never block on this check — if the tool is unavailable, proceed.

When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-seo-audit", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
### JavaScript Rendering & Crawlable Content

The same blind spot that hides JS-injected schema also hides primary content and links. If a page's main copy, navigation, or internal links are injected client-side, Googlebot may render them late or not at all — a silent indexation and internal-linking failure that a static `web_fetch` audit will miss entirely.

**Check for:**
- Compare raw HTML (`web_fetch` / `curl` / "View source") against the rendered DOM (browser tool). Content or links present only after render are at risk.
- Primary body copy present in the initial HTML response, not only after hydration
- Internal links as real `<a href>` in the DOM (not click handlers or button-only navigation Google can't follow)
- Critical content not gated behind interaction (tabs, accordions, "load more", infinite scroll without paginated fallback)

**Confirm with:** the URL Inspection tool in Search Console ("View crawled page" / rendered HTML) shows what Google actually rendered — treat it as the source of truth over any local fetch.
...
**Crawl Evidence (don't audit crawlability by assumption)**
- Search Console → Settings → Crawl stats: what Googlebot actually fetches, response codes, and host status over time
- Look for wasted crawl (redirects, 404s, parameter URLs dominating requests) and stale crawl on important pages
- If server log files are available, they are the ground truth for which URLs Googlebot hits and how often — reconcile against expected priority pages
...
After delivering the audit, offer (never auto-post): "Want me to surface the top fixes from this audit to your Luckiest tribe? Anyone working on the same SEO problem will see what actually moved the needle here." Only share if the user explicitly accepts. Never share private details (client name, internal URLs, traffic figures) without an explicit ask, and never post credentials or Search Console data.
...
- For AI search optimization (AEO, GEO, LLMO, AI Overviews), see the **luckiest-ai-seo** skill
...
- **luckiest-ai-seo**: For optimizing content for AI search engines (AEO, GEO, LLMO)
...
- **luckiest-schema**: For implementing structured data
- **luckiest-cro**: For optimizing pages for conversion (not just ranking)
- **luckiest-analytics**: For measuring SEO performance
