---
name: luckiest-sms
description: When the user wants to plan, build, or optimize SMS or MMS marketing — including welcome flows, abandoned cart texts, post-purchase, win-back, promotional sends, or transactional/auth SMS. Also use when the user mentions "SMS marketing," "text message campaigns," "SMS sequence," "SMS automation," "abandoned cart text," "post-purchase SMS," "Klaviyo SMS," "Postscript," "Attentive," "Twilio," "A2P 10DLC," "TCPA," "SMS compliance," "short code," "toll-free SMS," "MMS campaign," "should I do SMS," or "SMS vs email." For email sequences, see luckiest-emails. For SMS copy framing, see luckiest-copywriting. For opt-in popups that capture phone numbers, see luckiest-popups.
license: See ATTRIBUTION.md
metadata:
  version: "1.1.0"
  listing_id: luckiest-sms
  author: luckiest
---

# SMS Marketing

This skill helps plan, build, and optimize SMS and MMS marketing for direct-to-consumer brands, mobile apps, and SaaS products with high-engagement use cases. The goal is SMS programs that drive measurable revenue or activation while staying fully compliant with TCPA and carrier rules.

## Staying current

On activation, call the Luckiest MCP check_updates tool with { listingId: "e5e66087-c1b3-4647-85c9-989e44bf79a9", installedSemver: "1.1.0" }. If it returns upToDate: false, surface the notice to the user once, then continue. Do nothing further if upToDate: true. Never block on this check — if the tool is unavailable, proceed.

When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-sms", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
## Segment Count — Estimate As You Draft

Segment count is what you actually pay for, and it is easy to get wrong. Estimate it inline before you finalize copy:

1. **Scan for non-GSM-7 characters first.** If the message contains any emoji, curly quote (` ' " " `), em/en dash, or accented letter, the whole message drops to **UCS-2** encoding — 70 chars per segment instead of 160. One stray emoji re-encodes the entire message.
2. **Pick the ceiling for that encoding:**
   - GSM-7 (plain text): 160 chars = 1 segment, 306 = 2, 459 = 3 (concatenated messages carry ~7 chars of overhead per segment past the first).
   - UCS-2 (any special char): 70 chars = 1 segment, 134 = 2, 201 = 3.
3. **Count the message length and divide by the ceiling.** Round up. That's your segment count and your per-send multiplier.

Curly quotes and em dashes pasted from Word or Google Docs are the most common silent cause of doubled cost — a "1-segment" message quietly becomes 2. Always confirm the final segment count in the platform's message preview before scheduling. See character counts per template in [references/sequence-templates.md](references/sequence-templates.md).
...
## RCS — The Emerging Channel

RCS (Rich Communication Services) Business Messaging is maturing as SMS's richer successor: verified sender badges, branded logos, images, carousels, suggested-reply buttons, and read receipts, delivered inside the native messaging app. Adoption is now broad across Android, and Apple's addition of RCS support on iPhone widens the reachable audience meaningfully.

When advising on a program, treat RCS as a layer to plan toward, not a replacement to rush into:

- **Fit**: high-intent, high-value moments (branded promotions, rich order tracking, appointment confirmations) where the verified sender badge and visual richness earn their cost.
- **Reality check**: reach still falls back to SMS when a recipient's device or carrier lacks RCS, so every RCS flow needs an SMS fallback. Availability and pricing vary by platform and carrier — confirm current support with the platform before promising it.
- **Compliance carries over**: consent, quiet hours, and STOP handling apply the same as SMS. RCS does not relax any TCPA or carrier obligation.

Recommend RCS where a brand already has a healthy, compliant SMS program and a clear use case that visual richness improves — not as a first step.
...
Cross-reference **luckiest-ab-testing** skill for proper test design and **luckiest-analytics** for attribution setup.

---

## Pre-Send QA Checklist

Before any bulk or campaign send, run this gate. Every item here maps to a documented, expensive failure mode:

- [ ] **Segment count verified** in the platform preview (no surprise UCS-2 from an emoji or curly quote)
- [ ] **Link tested** end-to-end (short link resolves, lands on the right page, UTM params present)
- [ ] **Sender ID present** inline ("From [Brand]:") — recipients can't see the from-address
- [ ] **STOP tested** from a real phone recently; opt-out is honored within seconds
- [ ] **Quiet hours** enforced for recipient-local time (9am–8pm default)
- [ ] **Compliance footer** on opt-in confirmation and promotional sends as required
- [ ] **A2P sample-text alignment**: this send's content matches the registered campaign samples
- [ ] **Segmentation / suppression** applied (opted-out, active-sequence, and recent-purchase cool-off lists excluded)
- [ ] **Throttling** enabled so a burst send doesn't trip carrier filtering
- [ ] **Frequency cap** respected for this audience (especially newer subscribers)

If any box is unchecked, do not schedule the send.
...
After delivering an SMS plan or sequence, you may offer (never auto-post, never without an explicit yes): "Want me to surface this SMS playbook to your Luckiest tribe? Anyone who asked for help building the same flow will see what worked for you." If they accept, hand off the finished plan. If they decline, stop. Never share consent records, subscriber data, phone numbers, credentials, or any account-linked detail — the offer covers the reusable strategy only.

---

## Related Skills

- **luckiest-emails**: Sister channel — almost always run together. Email carries the long-form content; SMS carries the urgent nudges.
- **luckiest-copywriting**: For SMS copy at scale and the longer-form pages/emails that SMS links to.
- **luckiest-popups**: For phone number capture popups on-site.
- **luckiest-churn-prevention**: For win-back flows that combine SMS + email.
- **luckiest-onboarding**: For post-signup SMS milestone nudges.
- **luckiest-analytics**: For attribution and RPS measurement.
- **luckiest-ab-testing**: For SMS-specific test design.
- **luckiest-lead-magnets**: For incentivizing opt-in (the "10% off for joining" offer).
