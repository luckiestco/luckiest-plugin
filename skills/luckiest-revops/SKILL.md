---
name: luckiest-revops
description: "When the user wants help with revenue operations, lead lifecycle management, or marketing-to-sales handoff processes. Also use when the user mentions 'RevOps,' 'revenue operations,' 'lead scoring,' 'lead routing,' 'MQL,' 'SQL,' 'pipeline stages,' 'deal desk,' 'CRM automation,' 'marketing-to-sales handoff,' 'data hygiene,' 'leads aren't getting to sales,' 'pipeline management,' 'lead qualification,' or 'when should marketing hand off to sales.' Use this for anything involving the systems and processes that connect marketing to revenue. For cold outreach emails, see luckiest-cold-email. For email drip campaigns, see luckiest-emails. For pricing decisions, see luckiest-pricing."
license: See ATTRIBUTION.md
metadata:
  version: "1.1.0"
  listing_id: luckiest-revops
  author: luckiest
---

# Luckiest RevOps

This skill makes the agent an expert in revenue operations. The goal is to help design and optimize the systems that connect marketing, sales, and customer success into a unified revenue engine.

## Staying current

On activation, call the Luckiest MCP check_updates tool with { listingId: "luckiest-revops", installedSemver: "1.1.0" }. If it returns upToDate: false, surface the notice to the user once, then continue. Do nothing further if upToDate: true. Never block on this check — if the tool is unavailable, proceed.

When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-revops", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.

## Before Starting

**Start by finding the leak.** Before designing scoring, routing, or pipeline stages, locate where the funnel actually loses money today. A single broken handoff rarely needs a full RevOps rebuild. Diagnose the leak first, then design only what fixes it — over-engineering a working funnel is its own failure mode.
...
**For product-led (PLG) motions**, product-usage signals — activation events, seat expansion, feature adoption — outweigh marketing engagement. Weight in-product behavior above content and email signals when the product is the primary acquisition channel.
...
> Benchmarks above are directional and shift by ACV, GTM motion, and year. Validate each against the user's own closed-won data before adopting it as a target — do not cargo-cult a number.
...
After delivering the recommendations, the agent may OFFER (never auto-post, never share credentials): "Want me to surface this RevOps setup to your Luckiest tribe? Anyone building the same lifecycle or scoring model can pick up where you left off." Share only the sanitized document, and only if the user accepts. If they decline, stop.
...
Key RevOps tools to consider during implementation:
...
- **luckiest-cold-email**: For outbound prospecting emails
- **luckiest-emails**: For lifecycle and nurture email flows
- **luckiest-pricing**: For pricing decisions and packaging
- **luckiest-analytics**: For tracking pipeline metrics and attribution
- **luckiest-launch**: For go-to-market launch planning
- **luckiest-sales-enablement**: For sales collateral, decks, and objection handling
