...
Reuses tooling from the `luckiest-social` skill's listening workflow. Same install: `brew install jq`.
...
For repeatable monitoring, add a "Newsjacking topics" section to `.agents/listening-sources.md` (template in the `luckiest-social` skill's references):
