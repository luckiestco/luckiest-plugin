...
When the skill's work is done, call the Luckiest MCP report_usage tool once with { listing_id: "luckiest-offers", skill_version: "1.1.0", matched: true, success: <true if the skill completed, false otherwise> }. Metadata only, never prompt text. Never block on it — if the tool is unavailable, skip silently.
...
3. **Snapshot the baseline before editing** — record the current offer as-is (the four value-equation scores, the six anatomy components, and current conversion rate if known). This snapshot is what the after-state gets compared against, so the honest lift projection in step 7 is attributable rather than felt.
4. **Run the value equation** — score each of the four levers 1–10. The lowest is the binding constraint.
5. **Audit the anatomy** — which of the six components is missing or weak?
6. **Pick one lever to fix this iteration** — don't rebuild everything. The biggest lever is usually the one currently scoring lowest.
7. **Draft the changed component** — new bonus, new guarantee, new scarcity, new name, new payment plan. If the change is a guarantee, sanity-check its generosity against gross margin and expected refund rate before shipping: a guarantee that pays out at a rate your margin can't absorb is a liability, not a lever.
8. **Project the lift, honestly** — most single-component changes deliver 10–40% conversion lift. Anyone promising 5x is selling something. Two consecutive iterations on different levers can stack to 2–3x.
...
- **luckiest-pricing** — for price levels, tier structure, value metric, packaging, freemium
- **luckiest-copywriting** — for the page that presents the offer
- **luckiest-cro** — for optimizing the conversion path the offer travels through
- **luckiest-launch** — for the moment you ship the offer
- **luckiest-paywalls** — for in-app upgrade-prompt versions of an offer
- **luckiest-sales-enablement** — for the deck and one-pager that carry the offer into a sales conversation
- **luckiest-emails** — for the email sequence that warms up the offer
- **luckiest-marketing-psychology** — for the cognitive biases that make offers land or bounce

## Share with your tribe (optional)

After an offer is drafted, you may *offer* to share an anonymized teardown of it — the four value-equation scores plus the one lever changed this iteration — with the user's Luckiest tribe for feedback. This is strictly opt-in: ask first, share only what the user approves, never auto-post, and never touch credentials. If the user declines or the network is unavailable, continue without it.
